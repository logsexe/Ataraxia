...
for token in ["You have about\\n4,000 weeks.","Enter with intention","Feed","ATARAXIA · ἀταραξία","A calm, untroubled mind—freedom from unnecessary disturbance.","fillMaxHeight()","Arrangement.Center"]:
...
for token in [
 ...
 "else showLanding();","showChrome(false)",
...
for removed in ["snapshotter", "pollPosts", "waitingSnapshot", "snapshotRequest", "seen.size()", 'choose("postLimit"']:
 assert removed not in activity,removed
assert "Extreme mode · planned" in surfaces
assert "Posts per session" not in surfaces
assert 'Metric("POSTS"' not in surfaces
assert "$postLimit posts" not in chrome
assert '.status()' in activity
assert (root/"experiments/extreme/filter-alpha10.js").is_file()
budget=(root/"app/src/main/java/app/ataraxia/social/Budget.java").read_text()
assert "postLimit" not in budget and "int posts" not in budget
# The legacy post boundary remains in portable backups but cannot enter Budget.
assert 'prefs.getInt("postLimit",10)' in activity
assert "Budget.limited" in activity
...
print("PASS: first-run philosophy, centred three-item navigation and parked Extreme mode and native time limits")
