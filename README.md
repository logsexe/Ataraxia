...
This is a pilot, not a production release or independently audited app. Keep the official app until the pilot passes your tests. Builds made with different signing keys cannot update one another without uninstalling first. Uninstalling clears this app's login, settings and time usage; your Instagram account remains.
...
- Defaults: 5 browsing minutes per session, 15 browsing minutes per day, 10-minute cooldown after a timed session.
- Time usage persists locally across app restarts. A local calendar-day change resets the daily time allowance. Login and direct-message routes remain exempt.
- Post counting is parked for future **Extreme mode**, with the prototype preserved outside the APK. Normal browsing has no post quota or post-identifier polling. Existing backup post-boundary values are retained but inactive.
...
- Ad/suggestion detection depends on website markup. Some feeds may not expose supported article/permalink structures. Native time limits remain independent of that markup. No promise of zero ads or tracking.
...
- Filtering waits until a drag/fling has settled before changing layout. A newly labelled ad can remain visible while scrolling; native scroll anchoring handles the eventual collapse. No custom scroll-offset writes are used.
...
This project uses Gradle 8.9 / Android Gradle Plugin 8.7.3, Java 17, compile/target SDK 35, minimum Android 11 (API 30). The native UI uses AndroidX Activity and Compose Material 3. The debug variant is explicitly non-debuggable. Android Studio can create a release signing key for your own maintained distribution.
...
Browser fixtures: install Node.js and Playwright in a test environment (`npm install --no-save playwright` and `npx playwright install chromium`), then run `node tests/filter.test.cjs` and `node tests/feed-stability.test.cjs`. These are synthetic DOM fixtures, not evidence of live Instagram compatibility.

See `docs/ALPHA11-SCROLL.md` for this build's scope and `docs/ROADMAP.md` for following-only, friends-first and future Extreme-mode work.
